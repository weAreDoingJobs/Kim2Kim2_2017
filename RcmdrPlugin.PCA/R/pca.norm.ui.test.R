...
pca.norm.ui.test <- function(){
  library(SixSigma)
  initializeDialog(title = gettextRcmdr("Process Capability Analysis(Normal Distribution)"))
  #defaults
  defaults <- list(xLTnName = 40, xLTmeanName = 753, xLTsdName = 3,
                   lslName= 740, uslName = 760,tarName = 750)
  ...
  onOK <- function() {
    ...
    target <- round(as.numeric(tclvalue(tarName)))
    closeDialog()

    if (length(x) == 0) {
      UpdateModelNumber(-1)
      errorCondition(recall = pca.norm.ui.test, message = gettextRcmdr("You must select a variable."))
      return()
    }
    if (!is.numeric(nName) || nName <=0 ){

      errorCondition(recall = pca.norm.ui.test, message = gettextRcmdr("You must put number bigger than 0."))
      return()
    }
    if (!is.numeric(meanName)) {

      errorCondition(recall = pca.norm.ui.test, message = gettextRcmdr("You must put numeric value."))
      return()
    }
    if (!is.numeric(sdName) || sdName <= 0 ) {

      errorCondition(recall = pca.norm.ui.test, message = gettextRcmdr("You must put numeric value bigger than 0."))
      return()
    }
    if (!is.numeric(LSL)) {
    ...
    xLTName <- paste(nName,",",meanName,",",sdName,sep = "")
    dataName <- paste(ActiveDataSet(),"$",x,sep="")
    command <-paste("ss.study.ca( xST = ",dataName,",rnorm(",xLTName,"),LSL =",LSL,", USL = ",USL,",Target =",target,")", sep = "")
  ...
  tkgrid(labelRcmdr(tarFrame, text =    gettextRcmdr(" TAR  ")), tarField, sticky = "w")
